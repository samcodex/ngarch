...
export function forceInBox(alphaOfBoxForce) {
  function index(d) {
    return d.index;
  }
  ...
  function force(alpha = alphaOfBoxForce) {
    if (!enableGrouping) {
      return force;
    }
    if (template === 'force') {
  ...
  function computeClustersNodeCounts(clusterNodes) {
  ...
  function getGroupsGraph() {
    ...
    clustersLinks.forEach(function (l) {
      ...
      const target = dNodes.get(l.target);
      if (source !== undefined && target !== undefined) {
        glinks.push({
          'source': source,
          'target': target,
        ...
        console.log(`Force in a box error, couldn't find the link source or
          target on the list of nodes`);
      }
    });
  ...
  function checkLinksAsObjects() {
    ...
    links.forEach(function (link) {
      ...
      if (source === undefined || target === undefined) {
        console.log(link);
        throw Error(`Error setting links, couldn't find nodes for a link
  ...
  function initializeWithForce() {
    ...
    if (nodes && nodes.length > 0) {
      if (groupBy(nodes[0]) === undefined) {
        throw Error(`Couldn't find the grouping attribute for the nodes.
          Make sure to set it up with forceInABox.groupBy('attr') before calling .links()`);
      }
    }
    ...
    templateForce = d3.forceSimulation(net.nodes)
      .force('x', d3.forceX(boxSize[0] / 2).strength(0.5))
      .force('y', d3.forceY(boxSize[1]  / 2).strength(0.5))
  ...
  force.size = function(_) {
    return arguments.length ? (boxSize = _, force) : boxSize;
  };
